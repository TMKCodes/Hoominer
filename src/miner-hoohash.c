...
int start_mining_threads(StratumContext *ctx, MiningState *ms)
{
  ...
  /* GPU (OpenCL) threads: supported for both hoohash and pepepow.
  ...
  if (ctx->config->disable_gpu == false)
  {
    if (ctx->config->disable_opencl == false)
    {
      ...
      void *(*opencl_thread_fn)(void *) = mining_opencl_thread;
      if (strcmp(ctx->config->algorithm, "pepepow") == 0)
        opencl_thread_fn = mining_opencl_thread_pepepow;
      ...
      for (int i = 0; i < ms->num_opencl_threads; i++)
      {
        ...
        mt->threadIndex = i;
        mt->ctx = ctx;
        if (pthread_create(&ms->mining_opencl_threads[i], NULL, opencl_thread_fn, mt))
        {
          printf("start_mining_threads: Failed to create thread %d\n", i);
          free(mt);
          return 1;
