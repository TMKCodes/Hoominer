               ...
               cl_ulong nonce_mask, cl_ulong nonce_fixed, cl_device_id device)
{
  ...
  cl_ulong total_nonces_tried = 0;
  Result result = {0, {0}, {0}, {0}, {0}};
  cl_ulong random_type = RANDOM_TYPE_XOSHIRO;
  cl_ulong iteration = 0;

  // Convert timestamp to cl_long
  cl_long timestamp_cl = (cl_long)timestamp;

  while (iteration < 18446744073709551615UL)
  {
    total_nonces_tried += global_size;
    printf("Iteration %lu: Trying %lu nonces for %s (total: %lu)\n", iteration, global_size, testname, total_nonces_tried);
    ...
    for (size_t i = 0; i < global_size; i++)
    {
      ...
      for (int j = 0; j < 32; j++)
      {
        seed[j] = previous_header[j] ^ ((i + total_nonces_tried + iteration) & 0xFF);
      }
...
int main(int argc, char **argv)
{
  ...
  cl_long timestamp = 1725374568455;
  ...
  unsigned char previous_header3[DOMAIN_HASH_SIZE] = "asdfucknt_header_example_32byte!";
  ...
  if (run_kernel(context, queue, program, kernel, local_size, global_size, "Test 1", previous_header1, target1, matrix1, timestamp, nonce_mask, nonce_fixed, device) != 0)
  {
    printf("Test 1 failed\n");
  }
  printf("\nTest 2: Second previous header, second target\n");
  if (run_kernel(context, queue, program, kernel, local_size, global_size, "Test 2", previous_header2, target2, matrix2, timestamp, nonce_mask, nonce_fixed, device) != 0)
  {
